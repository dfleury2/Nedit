...
static NeRange* RangesNew(int n)
{
   NeRange* newRanges;
   ...
   if (n != 0)
   {
       ...
       */
      n = (n >= 256) ? ((n + 64) & ~63) : ((n + 16) & ~15);
      newRanges = (NeRange*)malloc(n * sizeof(NeRange));
      return newRanges;
   }
...
static NeRange* RangesRealloc(NeRange* ranges, int n)
{
   int size;
   NeRange* newRanges;
   ...
   if (n > 0)
   {
      ...
      n = (n >= 256) ? ((n + 64) & ~63) : ((n + 16) & ~15);
      size = n * sizeof(NeRange);
      newRanges = (NeRange*)(ranges != NULL
                           ? realloc((char*)ranges, size)
                           : malloc(size));
      return newRanges;
   }
   else
...
static NeRange* RangesFree(NeRange* ranges)
{
   free((char*) ranges);
...
static void rangesetRefreshAllRanges(NeRangeset* rangeset)
{
   int i;
...
{
   NeRange* ranges = rangeset->ranges;
   int start, end;
...
void RangesetInit(NeRangeset* rangeset, int label, Ne_Text_Buffer* buf)
{
   ...
   rangeset->ranges = (NeRange*)0;		/* the ranges table */
...
{
   NeRange* origRanges, *plusRanges, *newRanges, *oldRanges;
   ...
   if (nOrigRanges == 0)
   {
      ...
      memcpy(newRanges, plusRanges, nPlusRanges * sizeof(NeRange));
   ...
   while (nOrigRanges > 0 || nPlusRanges > 0)
   {
      ...
      {
         SWAPval(NeRange*, &origRanges, &plusRanges);
...
{
   NeRange* origRanges, *minusRanges, *newRanges, *oldRanges;
...
*/
void RangesetGetInfo(NeRangeset* rangeset, int* defined, int* label,
                     int* count, char** color, char** name, char** mode)
{
   ...
   else
   {
      ...
      *color = rangeset->color_name ? rangeset->color_name : (char*)"";
      *name = rangeset->name ? rangeset->name : (char*)"";
...
static NeRangeset* rangesetFixMaxpos(NeRangeset* rangeset, int ins, int del)
{
...
NeRangesetTable* RangesetTableAlloc(Ne_Text_Buffer* buffer)
{
   int i;
   NeRangesetTable* table = (NeRangesetTable*)malloc(sizeof(NeRangesetTable));
   ...
   table->n_set = 0;
   table->list[0] = '\0';
   /* NeRange sets must be updated before the text display callbacks are
...
//      BufRemoveModifyCB(table->buf, RangesetBufModifiedCB, table);
   ...
   return (NeRangesetTable*)0;
}

/*
** clone a ranges set.
*/
static void rangesetClone(NeRangeset* destRangeset, NeRangeset* srcRangeset)
{
   ...
   {
      destRangeset->color_name = (char*)malloc(strlen(srcRangeset->color_name) +1);
      strcpy(destRangeset->color_name, srcRangeset->color_name);
   }

   if (srcRangeset->name)
   {
      destRangeset->name = (char*)malloc(strlen(srcRangeset->name) + 1);
   ...
   if (srcRangeset->ranges)
   {
      ...
      memcpy(destRangeset->ranges, srcRangeset->ranges,
             srcRangeset->n_ranges * sizeof(NeRange));
   }
}
...
*/
NeRangesetTable* RangesetTableClone(NeRangesetTable* srcTable,
                                  Ne_Text_Buffer* destBuffer)
{
   NeRangesetTable* newTable = NULL;
   int i;
...
static void RangesetTableListSet(NeRangesetTable* table)
{
   int i;
...
static int activateRangeset(NeRangesetTable* table, int active)
{
   int depth, i, j;
...
static int deactivateRangeset(NeRangesetTable* table, int active)
{
   int depth, n, i, j;
...
void RangesetTableUpdatePos(NeRangesetTable* table, int pos, int ins, int del)
{
   int i;
   NeRangeset* p;
...
{
   NeRangesetTable* table = (NeRangesetTable*)cbArg;
...
int RangesetAssignColorName(NeRangeset* rangeset, char* color_name)
{
   ...
   if (color_name)
   {
      cp = (char*)malloc(strlen(color_name) + 1);
...
{
   NeRangeset* rangeset = &table->set[index];
...
int RangesetTableAssignColorPixel(NeRangesetTable* table, int index, Fl_Color color,
                                  int ok)
{
   NeRangeset* rangeset = &table->set[index];
   rangeset->color_set = ok ? 1 : -1;
...
static int rangesetWeightedAtOrBefore(NeRangeset* rangeset, int pos)
{
...
static NeRangeset* rangesetInsDelMaintain(NeRangeset* rangeset, int pos, int ins, int del)
{
...
static NeRangeset* rangesetInclMaintain(NeRangeset* rangeset, int pos, int ins, int del)
{
   int i, j, n, *rangeTable = (int*)rangeset->ranges;
   int end_del, movement;
...
static NeRangeset* rangesetDelInsMaintain(NeRangeset* rangeset, int pos, int ins, int del)
{
   int i, j, n, *rangeTable = (int*)rangeset->ranges;
   int end_del, movement;
...
static NeRangeset* rangesetExclMaintain(NeRangeset* rangeset, int pos, int ins, int del)
{
   int i, j, n, *rangeTable = (int*)rangeset->ranges;
   int end_del, movement;
...
static NeRangeset* rangesetBreakMaintain(NeRangeset* rangeset, int pos, int ins, int del)
{
   int i, j, n, *rangeTable = (int*)rangeset->ranges;
...
int RangesetInverse(NeRangeset* rangeset)
{
   int* rangeTable;
   ...
   rangeset->n_ranges = n / 2;
   rangeset->ranges = RangesRealloc((NeRange*)rangeTable, rangeset->n_ranges);
