...
void SetFontCB(Fl_Widget* w, void* data)
{
  ...
  if (!fontName.empty())
  {
   text->setFont(Ne_Font(fontName));
   w->redraw();
  }
}

// --------------------------------------------------------------------------
void InsertCB(Fl_Widget*, void* data)
{
   text->insert((const char*)data);
}
...
int main(int argc, char* argv[])
{
   ...
   if ( argc > 1 )
   {
      ...
      buffer.setAll(filebuffer.c_str());
   }
   ...
   menuBar.add("Insert/Some Text", 0, InsertCB, (void*)"Some text");

   //Ne_Text_Editor* text = Ne_Text_Editor::Create(0, 25, win.w(), win.h()  -25,
   text = Ne_Text_Editor::Create(0, 25, win.w(), win.h()  -25,
     60,
     &buffer,
