...
static void histogramCharacters(const char* string, int length, char hist[256], int init);
...
static void deleteRect(Ne_Text_Buffer* buf, int start, int end, int rectStart, int rectEnd, int* replaceLen, int* endPos);
static void insertCol(Ne_Text_Buffer* buf, int column, int startPos, const char* insText, int* nDeleted, int* nInserted, int* endPos);
static void overlayRect(Ne_Text_Buffer* buf, int startPos, int rectStart, int rectEnd, const char* insText, int* nDeleted, int* nInserted, int* endPos);
static void insertColInLine(const char* line, const char* insLine, int column, int insWidth, int tabDist, int useTabs, char nullSubsChar, char* outStr, int* outLen, int* endOffset);
static void deleteRectFromLine(const char* line, int rectStart, int rectEnd, int tabDist, int useTabs, char nullSubsChar, char* outStr, int* outLen, int* endOffset);
static void overlayRectInLine(const char* line, const char* insLine, int rectStart, int rectEnd, int tabDist, int useTabs, char nullSubsChar, char* outStr, int* outLen, int* endOffset);
...
static void redisplaySelection(Ne_Text_Buffer* buf, selection* oldSelection, selection* newSelection);
...
static void addPadding(char* string, int startIndent, int toIndent, int tabDist, int useTabs, char nullSubsChar, int* charsAdded);
static int searchForward(Ne_Text_Buffer* buf, int startPos, char searchChar, int* foundPos);
...
static void findRectSelBoundariesForCopy(Ne_Text_Buffer* buf, int lineStartPos, int rectStart, int rectEnd, int* selStart, int* selEnd);
...
Ne_Text_Buffer* BufCreatePreallocated(int requestedSize)
{
   ...
   buf = (Ne_Text_Buffer*)malloc(sizeof(Ne_Text_Buffer));
   buf->length = 0;
   buf->buf = (char*)malloc(requestedSize + PREFERRED_GAP_SIZE + 1);
...
void BufFree(Ne_Text_Buffer* buf)
{
   ...
   {
      free((char*)buf->modifyProcs);
      free((char*)buf->cbArgs);
   }
   if (buf->rangesetTable)
      RangesetTableFree(buf->rangesetTable);
   if (buf->nPreDeleteProcs != 0)
   {
      free((char*)buf->preDeleteProcs);
      free((char*)buf->preDeleteCbArgs);
   }
   free((char*)buf);
}
...
void BufSetAll(Ne_Text_Buffer* buf, const char* text)
{
   ...
   buf->buf = (char*)malloc(length + PREFERRED_GAP_SIZE + 1);
   ...
   void* cbArg)
{
   ...
   newModifyProcs = (bufModifyCallbackProc*)
      malloc(sizeof(bufModifyCallbackProc*) * (buf->nModifyProcs+1));
   newCBArgs = (void**)malloc(sizeof(void*) * (buf->nModifyProcs+1));
   ...
   void* cbArg)
{
   ...
   newModifyProcs = (bufModifyCallbackProc*)
      malloc(sizeof(bufModifyCallbackProc*) * (buf->nModifyProcs));
   newCBArgs = (void**)malloc(sizeof(void*) * (buf->nModifyProcs));
   ...
   void* cbArg)
{
   ...
   newPreDeleteProcs = (bufPreDeleteCallbackProc*)
      malloc(sizeof(bufPreDeleteCallbackProc*) * (buf->nPreDeleteProcs+1));
   newCBArgs = (void**)malloc(sizeof(void*) * (buf->nPreDeleteProcs+1));
   ...
   void* cbArg)
{
   ...
   newPreDeleteProcs = (bufPreDeleteCallbackProc*)
      malloc(sizeof(bufPreDeleteCallbackProc*) * (buf->nPreDeleteProcs));
   newCBArgs = (void**)malloc(sizeof(void*) * (buf->nPreDeleteProcs));
   ...
   int rectEnd, int* replaceLen, int* endPos)
{
   ...
   free(text);
   free(expText);
   outStr = (char*)malloc(len + nLines * MAX_EXP_CHAR_LEN * 2 + 1);
...
static void reallocateBuf(Ne_Text_Buffer* buf, int newGapStart, int newGapLen)
{
   ...
   newBuf = (char*)malloc(buf->length + newGapLen + 1);
