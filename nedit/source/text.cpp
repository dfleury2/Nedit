...
static void initialize(TextWidget request, TextWidget newWidget);
...
static void initialize(TextWidget request, TextWidget newWidget)
{
   XFontStruct* fs = newWidget->text.fontStruct;
   char* delimiters;
   textBuffer* buf;
   Pixel white, black;
   int textLeft;
   int charWidth = fs->max_bounds.width;
   int marginWidth = newWidget->text.marginWidth;
   int lineNumCols = newWidget->text.lineNumCols;

   /* Set the initial window size based on the rows and columns resources */
   if (request->core.width == 0)
      newWidget->core.width = charWidth * newWidget->text.columns + marginWidth*2 +
                              (lineNumCols == 0 ? 0 : marginWidth + charWidth * lineNumCols);
   if (request->core.height == 0)
      newWidget->core.height = (fs->ascent + fs->descent) * newWidget->text.rows +
                               newWidget->text.marginHeight * 2;
   ...
   white = WhitePixelOfScreen(XtScreen((Widget)newWidget));
   black = BlackPixelOfScreen(XtScreen((Widget)newWidget));
   if (newWidget->text.selectBGPixel == white &&
         newWidget->core.background_pixel == white &&
         newWidget->text.selectFGPixel == black &&
         newWidget->primitive.foreground == black)
   {
      newWidget->text.selectBGPixel = black;
      newWidget->text.selectFGPixel = white;
   }
   ...
   textLeft = newWidget->text.marginWidth +
              (lineNumCols == 0 ? 0 : marginWidth + charWidth * lineNumCols);
   newWidget->text.textD = TextDCreate((Widget)newWidget, newWidget->text.hScrollBar,
                                       newWidget->text.vScrollBar, textLeft, newWidget->text.marginHeight,
                                       newWidget->core.width - marginWidth - textLeft,
                                       newWidget->core.height - newWidget->text.marginHeight * 2,
                                       lineNumCols == 0 ? 0 : marginWidth,
                                       lineNumCols == 0 ? 0 : lineNumCols * charWidth,
                                       buf, newWidget->text.fontStruct, newWidget->core.background_pixel,
                                       newWidget->primitive.foreground, newWidget->text.selectFGPixel,
                                       newWidget->text.selectBGPixel, newWidget->text.highlightFGPixel,
                                       newWidget->text.highlightBGPixel, newWidget->text.cursorFGPixel,
                                       newWidget->text.lineNumFGPixel,
                                       newWidget->text.continuousWrap, newWidget->text.wrapMargin,
                                       newWidget->text.backlightCharTypes, newWidget->text.calltipFGPixel,
                                       newWidget->text.calltipBGPixel);
   ...
   delimiters = XtMalloc(strlen(newWidget->text.delimiters) + 4);
   sprintf(delimiters, "%s%s", " \t\n", newWidget->text.delimiters);
   newWidget->text.delimiters = delimiters;
   ...
   newWidget->text.textD->cursorOn = False;

   /* Initialize the widget variables */
   newWidget->text.autoScrollProcID = 0;
   newWidget->text.cursorBlinkProcID = 0;
   newWidget->text.dragState = NOT_CLICKED;
   newWidget->text.multiClickState = NO_CLICKS;
   newWidget->text.lastBtnDown = 0;
   newWidget->text.selectionOwner = False;
   newWidget->text.motifDestOwner = False;
   newWidget->text.emTabsBeforeCursor = 0;

#ifndef NO_XMIM
   /* Register the widget to the input manager */
   XmImRegister((Widget)newWidget, 0);
   /* In case some Resources for the IC need to be set, add them below */
   XmImVaSetValues((Widget)newWidget, NULL);
#endif

   XtAddEventHandler((Widget)newWidget, GraphicsExpose, True,
                     (XtEventHandler)redisplayGE, (Opaque)NULL);

   if (newWidget->text.hidePointer)
   {
      Display* theDisplay;
      Pixmap empty_pixmap;
      XColor black_color;
      ...
      if (empty_cursor == 0)
      {
         theDisplay = XtDisplay((Widget)newWidget);
         empty_pixmap = XCreateBitmapFromData(theDisplay,
                                              RootWindowOfScreen(XtScreen((Widget)newWidget)), empty_bits, 1, 1);
         XParseColor(theDisplay, DefaultColormapOfScreen(XtScreen((Widget)newWidget)),
                     "black", &black_color);
      ...
      XtAddEventHandler((Widget)newWidget, NEDIT_HIDE_CURSOR_MASK, False,
                         ...
                         TextWidget newWidget)
{
   ...
   if (newWidget->text.overstrike != current->text.overstrike)
   {
   ...
   if (newWidget->text.fontStruct != current->text.fontStruct)
   {
      if (newWidget->text.lineNumCols != 0)
         reconfigure = True;
      TextDSetFont(current->text.textD, newWidget->text.fontStruct);
   }

   if (newWidget->text.wrapMargin != current->text.wrapMargin ||
         newWidget->text.continuousWrap != current->text.continuousWrap)
      TextDSetWrapMode(current->text.textD, newWidget->text.continuousWrap,
                       newWidget->text.wrapMargin);
   ...
   if (newWidget->text.delimiters != current->text.delimiters)
   {
      char* delimiters = XtMalloc(strlen(newWidget->text.delimiters) + 4);
   ...
   if (newWidget->text.lineNumCols != current->text.lineNumCols || reconfigure)
   {
      int marginWidth = newWidget->text.marginWidth;
      int charWidth = newWidget->text.fontStruct->max_bounds.width;
      int lineNumCols = newWidget->text.lineNumCols;
      if (lineNumCols == 0)
      {
         TextDSetLineNumberArea(newWidget->text.textD, 0, 0, marginWidth);
         newWidget->text.columns = (newWidget->core.width - marginWidth*2) / charWidth;
      }
      else
      {
         TextDSetLineNumberArea(newWidget->text.textD, marginWidth,
                                charWidth * lineNumCols,
                                2*marginWidth + charWidth * lineNumCols);
         newWidget->text.columns = (newWidget->core.width - marginWidth*3 - charWidth
                                    * lineNumCols) / charWidth;
      }
   }

   if (newWidget->text.backlightCharTypes != current->text.backlightCharTypes)
   {
      TextDSetupBGClasses((Widget)newWidget, newWidget->text.backlightCharTypes,
                          &newWidget->text.textD->bgClassPixel, &newWidget->text.textD->bgClass,
                          newWidget->text.textD->bgPixel);
      redraw = True;
   }
