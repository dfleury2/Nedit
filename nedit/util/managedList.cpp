                         ...
                         void* setDialogDataArg, void (*freeItemCB)(void*))
{
   ...
   ac++;
   XtSetArg(al[ac], XmNitems, placeholderTable);
   ac++;
   XtSetArg(al[ac], XmNitemCount, 1);
   ac++;
   XtSetArg(al[ac], XmNtopAttachment, XmATTACH_FORM);
   ac++;
   XtSetArg(al[ac], XmNleftAttachment, XmATTACH_WIDGET);
   ac++;
   XtSetArg(al[ac], XmNleftWidget, rowCol);
   ac++;
...
static int incorporateDialogData(managedListData* ml, int listPos, int explicitRequest)
{
   ...
   item = (*ml->getDialogDataCB)(listPos == 1 ? NULL : ml->itemList[
                                    listPos-2], explicitRequest, &abort, ml->getDialogDataArg);
   if (abort)
      return False;
