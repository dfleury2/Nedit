...
static Range* RangesNew(int n)
{
   ...
   if (n != 0)
   {
       ...
       */
      n = (n >= 256) ? ((n + 64) & ~63) : ((n + 16) & ~15);
      newRanges = (Range*)malloc__(n * sizeof(Range));
      return newRanges;
   }
...
void RangesetEmpty(Rangeset* rangeset)
{
   ...
   free__(rangeset->color_name);
   free__(rangeset->name);
...
/*
** Allocate and initialise, or empty and free__ a ranges set table.
*/

RangesetTable* RangesetTableAlloc(Ne_Text_Buffer* buffer)
{
   int i;
   RangesetTable* table = (RangesetTable*)malloc__(sizeof(RangesetTable));
...
static void rangesetClone(Rangeset* destRangeset, Rangeset* srcRangeset)
{
   ...
   {
      destRangeset->color_name = (char*)malloc__(strlen(srcRangeset->color_name) +1);
      strcpy(destRangeset->color_name, srcRangeset->color_name);
   }

   if (srcRangeset->name)
   {
      destRangeset->name = (char*)malloc__(strlen(srcRangeset->name) + 1);
...
int RangesetAssignColorName(Rangeset* rangeset, char* color_name)
{
   ...
   if (color_name)
   {
      cp = (char*)malloc__(strlen(color_name) + 1);
      strcpy(cp, color_name);
   }
   else
      cp = color_name;

   /* free__ old color name value */
   free__(rangeset->color_name);
...
int RangesetAssignName(Rangeset* rangeset, char* name)
{
   ...
   if (name)
   {
      cp = (char*)malloc__(strlen(name) + 1);
      strcpy(cp, name);
   }
   else
   {
...
int RangesetTableAssignColorPixel(RangesetTable* table, int index, Fl_Color color,
                                  int ok)
{
   Rangeset* rangeset = &table->set[index];
   rangeset->color_set = ok ? 1 : -1;
