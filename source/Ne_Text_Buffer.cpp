...
char* BufGetAll(Ne_Text_Buffer* buf)
{
   ...
   text = (char*)malloc__(buf->length+1);
...
void BufSetAll(Ne_Text_Buffer* buf, const char* text)
{
   ...
   buf->buf = (char*)malloc__(length + PREFERRED_GAP_SIZE + 1);
   ...
   int rectEnd, const char* text)
{
   ...
   if (nInsertedLines < nDeletedLines)
   {
      ...
      insLen = strlen(text);
      insText = (char*)malloc__(insLen + nDeletedLines - nInsertedLines + 1);
      strcpy(insText, text);
   ...
   int rectEnd)
{
   ...
   newlineString = (char*)malloc__(nLines+1);
   for (i=0; i<nLines; i++)
      newlineString[i] = '\n';
   newlineString[i] = '\0';
   ...
   int rectStart, int rectEnd)
{
   ...
   textOut = (char*)malloc__((end - start) + 1);
   lineStart = start;
   outPtr = textOut;
   while (lineStart <= end)
   {
   ...
   const char* insText, int* nDeleted, int* nInserted, int* endPos)
{
   ...
   expText = expandTabs(insText, 0, buf->tabDist, buf->nullSubsChar,
      &expInsLen);
   free__(expText);
   outStr = (char*)malloc__(expReplLen + expInsLen +
   ...
   int rectEnd, int* replaceLen, int* endPos)
{
   ...
   free__(text);
   free__(expText);
   outStr = (char*)malloc__(len + nLines * MAX_EXP_CHAR_LEN * 2 + 1);
   ...
   int* nDeleted, int* nInserted, int* endPos)
{
   ...
   expText = expandTabs(insText, 0, buf->tabDist, buf->nullSubsChar,
      &expInsLen);
   free__(expText);
   outStr = (char*)malloc__(end-start + expInsLen +
...
static void reallocateBuf(Ne_Text_Buffer* buf, int newGapStart, int newGapLen)
{
   ...
   newBuf = (char*)malloc__(buf->length + newGapLen + 1);
   ...
   char nullSubsChar, int* newLen)
{
   ...
   outStr = (char*)malloc__(strlen(text)+1);
   outPtr = outStr;
   indent = startIndent;
   for (c=text; *c!='\0';)
   {
