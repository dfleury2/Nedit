...
#define TEXT_OF_TEXTD(t)    ((t)->text)

enum positionTypes {CURSOR_POS, CHARACTER_POS};

static void updateLineStarts(Ne_Text_Display* textD, int pos, int charsInserted, int charsDeleted, int linesInserted, int linesDeleted, int* scrolled);
static void offsetLineStarts(Ne_Text_Display* textD, int newTopLineNum);
static void calcLineStarts(Ne_Text_Display* textD, int startLine, int endLine);
static void calcLastChar(Ne_Text_Display* textD);
static int posToVisibleLineNum(Ne_Text_Display* textD, int pos, int* lineNum);
static void redisplayLine(Ne_Text_Display* textD, int visLineNum, int leftClip, int rightClip, int leftCharIndex, int rightCharIndex);
static void drawString(Ne_Text_Display* textD, int style, int x, int y, int toX, char* string, int nChars);
static void clearRect(Ne_Text_Display* textD, Fl_Color color, int x, int y, int width, int height);
static void drawCursor(Ne_Text_Display* textD, int x, int y);
static int styleOfPos(Ne_Text_Display* textD, int lineStartPos, int lineLen, int lineIndex, int dispIndex, int thisChar);
static int stringWidth(const Ne_Text_Display* textD, const char* string, const int length, const int style);
static int inSelection(selection* sel, int pos, int lineStartPos, int dispIndex);
static int xyToPos(Ne_Text_Display* textD, int x, int y, int posType);
static void xyToUnconstrainedPos(Ne_Text_Display* textD, int x, int y, int* row, int* column, int posType);
static void bufPreDeleteCB(int pos, int nDeleted, void* cbArg);
static void bufModifiedCB(int pos, int nInserted, int nDeleted, int nRestyled, const char* deletedText, void* cbArg);
static void setScroll(Ne_Text_Display* textD, int topLineNum, int horizOffset, int updateVScrollBar, int updateHScrollBar);
static void hScrollCB(Fl_Widget* w, void* data);
static void vScrollCB(Fl_Widget* w, void* data);
// TODO: static void visibilityEH(Fl_Widget* w, void* data, int event, bool* continueDispatch);
static void redrawLineNumbers(Ne_Text_Display* textD, int clearAll);
static void updateVScrollBarRange(Ne_Text_Display* textD);
static int updateHScrollBarRange(Ne_Text_Display* textD);
...
static void findWrapRange(Ne_Text_Display* textD, const char* deletedText, int pos, int nInserted, int nDeleted, int* modRangeStart, int* modRangeEnd, int* linesInserted, int* linesDeleted);
static void wrappedLineCounter(const Ne_Text_Display* textD, const Ne_Text_Buffer* buf, const int startPos, const int maxPos, const int maxLines,
                               const bool startPosIsLineStart, const int styleBufOffset,
                               int* retPos, int* retLines, int* retLineStart, int* retLineEnd);
static void findLineEnd(Ne_Text_Display* textD, int startPos, int startPosIsLineStart, int* lineEnd, int* nextLineStart);
static int wrapUsesCharacter(Ne_Text_Display* textD, int lineEndPos);
...
static void extendRangeForStyleMods(Ne_Text_Display* textD, int* start, int* end);
static int getAbsTopLineNum(Ne_Text_Display* textD);
static void offsetAbsLineNum(Ne_Text_Display* textD, int oldFirstChar);
static int maintainingAbsTopLineNum(Ne_Text_Display* textD);
static void resetAbsLineNum(Ne_Text_Display* textD);
static int measurePropChar(const Ne_Text_Display* textD, const char c, const int colNum, const int pos);
static Fl_Color allocBGColor(Fl_Widget* w, char* colorName, int* ok);
static Fl_Color getRangesetColor(Ne_Text_Display* textD, int ind, Fl_Color bground);
static void textDRedisplayRange(Ne_Text_Display* textD, int start, int end);
   ...
   : Fl_Group(x, y, w, h, l), canRedraw(false), text(this)
{
   ...
   this->selectBGPixel = GetColor(NEDIT_DEFAULT_SEL_BG);
   this->selectFGPixel = GetColor(NEDIT_DEFAULT_SEL_FG);
   ...
   if (this->nVisibleLines == 0) ++this->nVisibleLines; // Avoid 0 array allocation
   this->lineStarts = (int*)malloc__(sizeof(int) * this->nVisibleLines);
   this->lineStarts[0] = 0;
...
void Ne_Text_Display::draw()
{
   ...
   TextDRedisplayRect(this, lineNumLeft, top, this->width + this->left - lineNumLeft, this->height);
...
// TODO:    XtAddEventHandler(widget, VisibilityChangeMask, false, visibilityEH, textD);
...
void TextDFree(Ne_Text_Display* textD)
{
   ...
   free__((char*)textD->lineStarts);
   free__((char*)textD->bgClassPixel);
   free__((char*)textD->bgClass);
// TODO:    free__((char*)textD);
}
...
// TODO:    XChangeGC(d, textD->cursorFGGC, GCForeground, &values);
   ...
   /* Redisplay */
   textD->redraw();
// TODO:    TextDRedisplayRect(textD, textD->left, textD->top, textD->width, textD->height);
...
void TextDSetFont(Ne_Text_Display* textD, const Ne_Font& fontStruct)
{
   ...
   Fl_Color bgPixel, fgPixel, selectFGPixel, selectBGPixel;
   Fl_Color highlightFGPixel, highlightBGPixel, lineNumFGPixel;
   Ne_Font* styleFont;
   ...
   fontWidth = fontStruct.max_width();
   if (fontWidth != fontStruct.min_width())
      fontWidth = -1;
   else
   {
      for (i = 0; i < textD->nStyles; i++)
...
// TODO:    XGetGCValues(display, textD->selectGC, GCForeground | GCBackground, &values);
   selectFGPixel = textD->selectFGPixel;
   selectBGPixel = textD->selectBGPixel;
// TODO:    XGetGCValues(display, textD->highlightGC, GCForeground | GCBackground, &values);
   highlightFGPixel = textD->highlightFGPixel;
...
// TODO:    clearRect(textD, bgPixel, textD->left,
...
int TextDMinFontWidth(Ne_Text_Display* textD, bool considerStyles)
{
   ...
   if (considerStyles)
   {
      for (i = 0; i < textD->nStyles; ++i)
      {
         int thisWidth = (textD->styleTable[i].font).min_width();
...
int TextDMaxFontWidth(Ne_Text_Display* textD, bool considerStyles)
{
   ...
   if (considerStyles)
   {
      for (i = 0; i < textD->nStyles; ++i)
      {
         int thisWidth = (textD->styleTable[i].font).max_width();
         if (thisWidth > fontWidth)
         {
            fontWidth = thisWidth;
         }
...
void TextDResize(Ne_Text_Display* textD, int width, int height)
{
   ...
   {
      free__((char*)textD->lineStarts);
      textD->lineStarts = (int*)malloc__(sizeof(int) * newVisibleLines);
   }
...
// TODO:                  height - exactHeight, false);
   ...
   if ( oldVisibleLines < newVisibleLines && textD->topLineNum + textD->nVisibleLines > textD->nBufferLines)
      setScroll(textD, max(1, textD->nBufferLines - textD->nVisibleLines + 2 + TEXT_OF_TEXTD(textD).cursorVPadding), textD->horizOffset, false, false);
...
void TextDBlankCursor(Ne_Text_Display* textD)
{
   ...
   textD->redraw();// TODO: textDRedisplayRange(textD, textD->cursorPos - 1, textD->cursorPos + 1);
}
...
void TextDSetCursorStyle(Ne_Text_Display* textD, int style)
{
   ...
   blankCursorProtrusions(textD);
   if (textD->cursorOn && textD->canRedraw)
   {
      textD->redraw(); // TODO: textDRedisplayRange(textD, textD->cursorPos - 1, textD->cursorPos + 1);
   }
}
...
void TextDOverstrike(Ne_Text_Display* textD, char* text)
{
   ...
   for (p = startPos; ; p++)
   {
      ...
      else if (indent > endIndent)
      {
         ...
         {
            p++;
            paddedText = (char*)malloc__(textLen + MAX_EXP_CHAR_LEN + 1);
...
*/
void TextDXYToUnconstrainedPosition(Ne_Text_Display* textD, int x, int y, int* row,
                                    int* column)
{
...
int TextDLineAndColToPos(Ne_Text_Display* textD, int lineNum, int column)
{
   ...
   /* int is the start of the line plus the index into line buffer */
...
int TextDOffsetWrappedRow(Ne_Text_Display* textD, int row)
{
   if (!textD->continuousWrap || row < 0 || row > textD->nVisibleLines)
      return row;
   return BufCountLines(textD->buffer, textD->firstChar, textD->lineStarts[row]);
}
...
*/
int TextDCountLines(Ne_Text_Display* textD, int startPos, int endPos,
...
** by avoiding the additional step of scanning back to the last newline.
*/
int TextDCountForwardNLines(const Ne_Text_Display* textD, const int startPos,
                            const unsigned nLines, const bool startPosIsLineStart)
{
   int retLines, retPos, retLineStart, retLineEnd;
...
** visLineLength.
*/
int TextDEndOfLine(const Ne_Text_Display* textD, const int pos,
                   const bool startPosIsLineStart)
{
   int retLines, retPos, retLineStart, retLineEnd;
...
static void bufPreDeleteCB(int pos, int nDeleted, void* cbArg)
{
   Ne_Text_Display* textD = (Ne_Text_Display*)cbArg;
   if (textD->continuousWrap &&
...
static void bufModifiedCB(int pos, int nInserted, int nDeleted, int nRestyled, const char* deletedText, void* cbArg)
{
   ...
   if (linesInserted == linesDeleted)
   {
      ...
      {
         endDispPos = textD->continuousWrap ? wrapModEnd : BufEndOfLine(buf, pos + nInserted) + 1;
...
static void drawString(Ne_Text_Display* textD, int style, int x, int y, int toX, char* string, int nChars)
{
   ...
   char buffer[4096] = "";
   strncat(buffer, string, nChars);

   /* Don't draw if widget isn't realized */
   if (!textD->canRedraw)
      return;

   bool isStyleGC = false;
   ...
   else if (style & HIGHLIGHT_MASK)
   {
      ...
      fground = textD->highlightFGPixel;
   }
   else if (style & PRIMARY_MASK)
   {
      bground = textD->selectBGPixel;
      fground = textD->selectFGPixel;
   }
   else
   {
      bground = textD->bgPixel;
...
// TODO:       XChangeGC(XtDisplay(textD->w), gc, GCFont | GCForeground | GCBackground, &gcValues);
   }
   ...
   if (style & FILL_MASK)
   {
      ...
      if (toX >= textD->left)
         clearRect(textD, bground, max(x, textD->left), y, toX - max(x, textD->left), textD->ascent + textD->descent);
      return;
   }
...
// TODO:       clearRect(textD, bgGC, x, y + textD->ascent + fs->descent, toX - x, textD->descent - fs->descent);
...
// TODO:    }
   ...
   fl_rectf(x, y, (int)fl_width(string, nChars), textD->ascent + textD->descent, bground); // ??? 
   fl_color(fground);
   fl_font(fs->font, fs->size);
   fl_draw(string, nChars, x, y + textD->ascent);
   ...
   if (style & SECONDARY_MASK || underlineStyle)
   {
      ...
      /* draw underline */
      fl_line(x, y + textD->ascent, toX - 1, y + textD->ascent);
   }
}
...
static void clearRect(Ne_Text_Display* textD, Fl_Color color, int x, int y, int width, int height)
{
   ...
   if (width == 0 && textD->canRedraw) // TODO: || XtWindow(textD->w) == 0)
      return;
...
static void drawCursor(Ne_Text_Display* textD, int x, int y)
{
   ...
   if (!textD->canRedraw || x < textD->left - 1 || x > textD->left + textD->width)
      return;
   ...
   fl_color(textD->cursorFGPixel);

   fl_push_no_clip();

   // Create segments and draw cursor
   if (textD->cursorStyle == NE_CARET_CURSOR)
   {
      int midY = bot - fontHeight / 5;
      fl_line(left, bot, x, midY, right, bot);
      fl_line(left, bot, x, midY-2, right, bot);
   }
   else if (textD->cursorStyle == NE_NORMAL_CURSOR)
   {
      fl_line(left, y, right, y);
      fl_line(x, y, x, bot);
      fl_line(left, bot, right, bot);
   }
   else if (textD->cursorStyle == NE_HEAVY_CURSOR)
   {
      fl_rect(x-1, y, 2, fontHeight);
      fl_rect(left, y, cursorWidth, 2);
      fl_rect(left, bot-1, cursorWidth, 2);
   }
    else if (textD->cursorStyle == NE_DIM_CURSOR)
    {
       fl_line(x, y, x, bot);
    }
   else if (textD->cursorStyle == NE_BLOCK_CURSOR)
   {
      right = x + fontWidth;
...
** be more appropriate.
*/
static int styleOfPos(Ne_Text_Display* textD, int lineStartPos,
                      int lineLen, int lineIndex, int dispIndex, int thisChar)
{
   Ne_Text_Buffer* buf = textD->buffer;
   Ne_Text_Buffer* styleBuf = textD->styleBuffer;
   int pos, style = 0;
...
*/
static int stringWidth(const Ne_Text_Display* textD, const char* string,
...
*/
static void xyToUnconstrainedPos(Ne_Text_Display* textD, int x, int y, int* row,
...
*/
static void updateLineStarts(Ne_Text_Display* textD, int pos, int charsInserted,
...
// TODO:    int xOffset, yOffset, srcX, srcY, dstX, dstY, width, height;
   ...
   if (textD->horizOffset == horizOffset && textD->topLineNum == topLineNum)
      return;
...
//// TODO:       XCopyArea(XtDisplay(textD->w), XtWindow(textD->w), XtWindow(textD->w), textD->gc, srcX, srcY, width, height, dstX, dstY);
...
static void updateVScrollBarRange(Ne_Text_Display* textD)
{
   ...
   textD->vScrollBar->value(sliderValue, sliderSize, 1, sliderMax);
}
...
static int updateHScrollBarRange(Ne_Text_Display* textD)
{
   ...
   if (textD->hScrollBar == NULL || !textD->hScrollBar->visible())
      return false;
   ...
   textD->hScrollBar->value(textD->horizOffset, sliderWidth, 1, sliderMax);
...
static void redrawLineNumbers(Ne_Text_Display* textD, int clearAll)
{
   ...
   if (clearAll)
   {
      ...
      fl_rectf(textD->lineNumLeft, textD->top, textD->lineNumWidth, textD->height);
   }
   ...
   for (visLine = 0; visLine < textD->nVisibleLines; visLine++)
   {
      ...
      if (lineStart != -1 && (lineStart == 0 || BufGetCharacter(textD->buffer, lineStart - 1) == '\n'))
      {
      ...
      {
         fl_rectf(textD->lineNumLeft, y, textD->lineNumWidth, textD->ascent + textD->descent, textD->bgPixel);
         if (visLine == 0)
            line++;
      }
      y += lineHeight;
...
static void vScrollCB(Fl_Widget* w, void* data)
{
   Ne_Text_Display* textD = (Ne_Text_Display*)data;
   int newValue = textD->vScrollBar->value();
...
{
   Ne_Text_Display* textD = (Ne_Text_Display*)data;
   int newValue = textD->hScrollBar->value();
...
// TODO: static void visibilityEH(Fl_Widget* w, void* data, int event,
...
// TODO:    ((Ne_Text_Display*)data)->visibility = ((XVisibilityEvent*)event)->state;
// TODO: }
   ...
   else
   {
      for (i = 0; i < lineLen; i++)
      {
         ...
         style = (unsigned char)BufGetCharacter(textD->styleBuffer,
                                                lineStartPos + i) - 'A';
         fl_font(textD->styleTable[style].font.font, textD->styleTable[style].font.size);
         width += (int)fl_width(expandedChar, len);
         charCount += len;
      }
   }
...
*/
static void findWrapRange(Ne_Text_Display* textD, const char* deletedText, int pos,
                          int nInserted, int nDeleted, int* modRangeStart, int* modRangeEnd,
                          int* linesInserted, int* linesDeleted)
{
   int length, retPos, retLines, retLineStart, retLineEnd;
   Ne_Text_Buffer* deletedTextBuf, *buf = textD->buffer;
   int nVisLines = textD->nVisibleLines;
   int* lineStarts = textD->lineStarts;
...
** Returned values:
**
**   retPos:	    int where counting ended.  When counting lines, the
...
*/
static void wrappedLineCounter(const Ne_Text_Display* textD, const Ne_Text_Buffer* buf,
...
*/
static int measurePropChar(const Ne_Text_Display* textD, const char c,
                           const int colNum, const int pos)
{
   ...
   else
   {
      ...
      {
         /* encountered "unfinished" style, trigger parsing */
         (*textD->unfinishedHighlightCB)((Ne_Text_Display*) /* const cast*/ textD, pos, textD->highlightCBArg);
         style = (unsigned char)BufGetCharacter(styleBuf, pos);
      }
   }
...
*/
static void findLineEnd(Ne_Text_Display* textD, int startPos, int startPosIsLineStart,
...
static Fl_Color getRangesetColor(Ne_Text_Display* textD, int ind, Fl_Color bground)
{
   ...
   if (ind > 0)
   {
      ...
      if (valid == 0)
      {
         ...
         if (color_name)
            color = allocBGColor(textD, color_name, &valid);
...
*/
void TextDSetupBGClasses(Fl_Widget* w, const char* str, Fl_Color** pp_bgClassPixel,
                         unsigned char** pp_bgClass, Fl_Color bgPixelDefault)
{
   ...
   free__((char*)*pp_bgClass);
   free__((char*)*pp_bgClassPixel);
   ...
   *pp_bgClass = NULL;
   ...
   while (s && class_no < 255)
   {
      ...
      if ((is_good = (is_good && *pos == ':')))
      {
         ...
         bgClassPixel[class_no] = GetColor(pos);
      }
      if (!is_good)
      {
   ...
   *pp_bgClass = (unsigned char*)malloc__(256);
   *pp_bgClassPixel = (Fl_Color*)malloc__(class_no * sizeof(Fl_Color));
