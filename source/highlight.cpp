...
/* Data structure attached to window to hold all syntax highlighting information (for both drawing and incremental reparsing) */
struct windowHighlightData
{
   ...
   NeStyleTableEntry* styleTable;
   int nStyles;
   Ne_Text_Buffer* styleBuffer;
...
static highlightDataRec* compilePatterns(Fl_Widget* dialogParent, highlightPattern* patternSrc, int nPatterns);
static void freePatterns(highlightDataRec* patterns);
static void handleUnparsedRegion(const WindowInfo* win, Ne_Text_Buffer* styleBuf, const int pos);
static void handleUnparsedRegionCB(Ne_Text_Display* textD, int pos, void* cbArg);
static void incrementalReparse(windowHighlightData* highlightData, Ne_Text_Buffer* buf, int pos, int nInserted, const char* delimiters);
static int parseBufferRange(highlightDataRec* pass1Patterns,
                            highlightDataRec* pass2Patterns, Ne_Text_Buffer* buf, Ne_Text_Buffer* styleBuf,
...
static void modifyStyleBuf(Ne_Text_Buffer* styleBuf, char* styleString,
                           int startPos, int endPos, int firstPass2Style);
static int lastModified(Ne_Text_Buffer* styleBuf);
static char getPrevChar(Ne_Text_Buffer* buf, int pos);
static regexp* compileREAndWarn(Fl_Widget* parent, const char* re);
...
static int findSafeParseRestartPos(Ne_Text_Buffer* buf, windowHighlightData* highlightData, int* pos);
static int backwardOneContext(Ne_Text_Buffer* buf, reparseContext* context, int fromPos);
static int forwardOneContext(Ne_Text_Buffer* buf, reparseContext* context, int fromPos);
...
static NeStyleTableEntry* styleTableEntryOfCode(WindowInfo* window, int hCode);
...
void SyntaxHighlightModifyCB(int pos, int nInserted, int nDeleted,  int nRestyled, const char* deletedText, void* cbArg)
{
   ...
   *highlightData = (windowHighlightData*)window->highlightData;
   ...
   if (nInserted > 0)
   {
      ...
      insStyle = (char*)malloc__(sizeof(char) * (nInserted + 1));
...
void StartHighlighting(WindowInfo* window, int warn)
{
   ...
   stylePtr = styleString = new char[window->buffer->length + 1];
...
/*
** Turn off syntax highlighting and free__ style buffer, compiled patterns, and
** related data.
*/
...
// TODO:    ((TextWidget)window->textArea)->text.textD->styleBuffer = NULL;
// TODO:    for (int i=0; i<window->nPanes; i++)
// TODO:       ((TextWidget)window->textPanes[i])->text.textD->styleBuffer = NULL;
}
...
void AttachHighlightToWidget(Fl_Widget* widget, WindowInfo* window)
{
   ...
   TextDAttachHighlightData(((Ne_Text_Editor*)widget),
...
{
   TextDAttachHighlightData(((Ne_Text_Editor*)widget), NULL, NULL, 0, UNFINISHED_STYLE, NULL, NULL);
}
...
static patternSet* findPatternsForWindow(WindowInfo* window, int warn)
{
   ...
   {
      if (warn)
         DialogF(DF_WARN, window->mainWindow, 1, "Language Mode",
...
** are encountered, warns user with a dialog and returns NULL.  To free__ the
...
static windowHighlightData* createHighlightData(WindowInfo* window, patternSet* patSet)
{
   ...
   NeStyleTableEntry* styleTable, *styleTablePtr;
   Ne_Text_Buffer* styleBuf;
   ...
   {
      DialogF(DF_WARN, window->mainWindow, 1, "Highlight Style", "Highlight style \"Plain\" is missing", "OK");
      return NULL;
   }

   for (i=0; i<nPatterns; i++)
   {
      if (patternSrc[i].subPatternOf != NULL && indexOfNamedPattern(patternSrc, nPatterns, patternSrc[i].subPatternOf) == -1)
      {
         DialogF(DF_WARN, window->mainWindow, 1, "Parent Pattern",
   ...
   for (i=0; i<nPatterns; i++)
   {
      ...
      {
         DialogF(DF_WARN, window->mainWindow, 1, "Highlight Style",
   ...
   {
      pass1Pats = compilePatterns(window->mainWindow, pass1PatternSrc, nPass1Patterns);
      if (pass1Pats == NULL)
         return NULL;
   }
   if (nPass2Patterns == 0)
   ...
   {
      pass2Pats = compilePatterns(window->mainWindow, pass2PatternSrc, nPass2Patterns);
      if (pass2Pats == NULL)
         return NULL;
   }
   ...
   parentStylesPtr = parentStyles = (char*)malloc__(nPass1Patterns+nPass2Patterns+2);
   *parentStylesPtr++ = '\0';
   *parentStylesPtr++ = '\0';
   for (i=1; i<nPass1Patterns; i++)
   {
   ...
   styleTablePtr = styleTable = new NeStyleTableEntry[nPass1Patterns + nPass2Patterns + 1];
#define setStyleTablePtr(styleTablePtr, patternSrc) \
    do { \
      NeStyleTableEntry *p = styleTablePtr; \
      ...
      p->color = GetColor(p->colorName); \
      if (p->bgColorName) { \
        p->bgColor = GetColor(p->bgColorName); \
      } \
      else { \
        p->bgColor = p->color; \
      } \
...
static highlightDataRec* compilePatterns(Fl_Widget* dialogParent, highlightPattern* patternSrc, int nPatterns)
{
   ...
   /* Allocate memory for the compiled patterns.  The list is terminated by a record with style == 0. */
   compiledPats = (highlightDataRec*)malloc__(sizeof(highlightDataRec) * (nPatterns + 1));
   ...
   for (i=0; i<nPatterns; i++)
      compiledPats[i].subPatterns = compiledPats[i].nSubPatterns == 0 ?
                                    NULL : (highlightDataRec**)malloc__( sizeof(highlightDataRec*) * compiledPats[i].nSubPatterns);
   for (i=0; i<nPatterns; i++)
      compiledPats[i].nSubPatterns = 0;
   for (i=1; i<nPatterns; i++)
   {
   ...
   for (patternNum=0; patternNum<nPatterns; patternNum++)
   {
      ...
      bigPattern = (char*)malloc__(sizeof(char) * (length+1));
      ptr=bigPattern;
      if (patternSrc[patternNum].endRE != NULL)
      {
         *ptr++ = '(';
...
static void freePatterns(highlightDataRec* patterns)
{
   ...
   for (i=0; patterns[i].style!=0; i++)
   {
      if (patterns[i].startRE != NULL)
         free__((char*)patterns[i].startRE);
      if (patterns[i].endRE != NULL)
         free__((char*)patterns[i].endRE);
      if (patterns[i].errorRE != NULL)
         free__((char*)patterns[i].errorRE);
      if (patterns[i].subPatternRE != NULL)
         free__((char*)patterns[i].subPatternRE);
   }

   for (i=0; patterns[i].style!=0; i++)
   {
      free__((char*) patterns[i].subPatterns);
   }

   free__((char*)patterns);
}
...
{
   NeStyleTableEntry* entry = styleTableEntryOfCode(window, hCode);
   return entry ? entry->highlightName : (char*)"";
}

char* HighlightStyleOfCode(WindowInfo* window, int hCode)
{
   NeStyleTableEntry* entry = styleTableEntryOfCode(window, hCode);
   return entry ? entry->styleName : (char*)"";
}
...
Fl_Color HighlightColorValueOfCode(WindowInfo* window, int hCode, int* r, int* g, int* b)
{
   ...
   {
      uchar rr, gg, bb;
      Fl::get_color(entry->color, rr, gg, bb);
      *r = rr;
      *g = gg;
      *b = bb;
      return entry->color;
   ...
   else
   {
      ...
      //*r = *g = *b = 0;
      //XtVaGetValues(window->textArea, XtNcolormap,   &cMap, XtNforeground, &colorDef.pixel, NULL);
...
Fl_Color GetHighlightBGColorOfCode(WindowInfo* window, int hCode, int* r, int* g, int* b)
{
   ...
   {
      uchar rr, gg, bb;
      Fl::get_color(entry->bgColor, rr, gg, bb);
      *r = rr;
      *g = gg;
      *b = bb;
   ...
   {
// TODO:       /* pick up background color of the (first) text widget of the window */
// TODO:       XColor colorDef;
// TODO:       Colormap cMap;
// TODO:       Display* display = XtDisplay(window->textArea);
// TODO:       *r = *g = *b = 0;
// TODO:       XtVaGetValues(window->textArea,
// TODO:                     XtNcolormap,   &cMap,
// TODO:                     XtNbackground, &colorDef.pixel,
// TODO:                     NULL);
// TODO:       if (XQueryColor(display, cMap, &colorDef))
// TODO:       {
// TODO:      uchar rr, gg, bb;
// TODO:      Fl::get_color(entry->bgColor, rr, gg, bb);
// TODO:          *r = rr;
// TODO:          *g = gg;
// TODO:          *b = bb;
// TODO:       }
// TODO:       return colorDef.pixel;
      return 0;
   }
}
...
*/
static void handleUnparsedRegion(const WindowInfo* window, Ne_Text_Buffer* styleBuf, const int pos)
{
   ...
   parseString(pass2Patterns, &stringPtr, &stylePtr, endParse - beginSafety, &prevChar, false, GetWindowDelimiters(window), string, NULL);
   ...
   BufReplace(styleBuf, beginParse, endParse, &styleString[beginParse-beginSafety]);
   free__(styleString);
   free__(string);
}
...
static void incrementalReparse(windowHighlightData* highlightData,
                               Ne_Text_Buffer* buf, int pos, int nInserted, const char* delimiters)
{
   int beginParse, endParse, endAt, lastMod, parseInStyle, nPasses;
   Ne_Text_Buffer* styleBuf = highlightData->styleBuffer;
      ...
      for (i=0; i<subPat->nSubPatterns; i++)
      {
         ...
         if (subSubPat->colorOnly)
         {
            if (!subExecuted)
            {
               ...
               {
                  fprintf(stderr, "Internal error, failed to recover " "start match in parseString\n");
                  return false;
               }
               subExecuted = true;
            }
...
*/
static int findSafeParseRestartPos(Ne_Text_Buffer* buf,
...
static int backwardOneContext(Ne_Text_Buffer* buf, reparseContext* context, int fromPos)
{
   if (context->nLines == 0)
      ...
      return max(0, BufCountBackwardNLines(buf, fromPos, context->nLines-1) - 1);
   else
      return max(0, min(max(0, BufCountBackwardNLines(buf, fromPos, context->nLines-1) -1), fromPos - context->nChars));
}
...
static int forwardOneContext(Ne_Text_Buffer* buf, reparseContext* context, int fromPos)
{
   if (context->nLines == 0)
      ...
      return min(buf->length, BufCountForwardNLines(buf, fromPos, context->nLines));
   else
      return min(buf->length, max(BufCountForwardNLines(buf, fromPos, context->nLines), fromPos + context->nChars));
}
...
static void updateWindowHeight(WindowInfo* window, int oldFontHeight)
{
// TODO:    int i, borderHeight, marginHeight;
// TODO:    int windowHeight, textAreaHeight, textHeight, newWindowHeight;
...
// TODO:    /* Decompose the window height into the part devoted to displaying
// TODO:       text (textHeight) and the non-text part (boderHeight) */
// TODO:    XtVaGetValues(window->mainWindow, XmNheight, &windowHeight, NULL);
// TODO:    XtVaGetValues(window->textArea, XmNheight, &textAreaHeight,
// TODO:                  textNmarginHeight, &marginHeight, NULL);
// TODO:    textHeight = textAreaHeight - 2*marginHeight;
// TODO:    for (i=0; i<window->nPanes; i++)
// TODO:    {
// TODO:       XtVaGetValues(window->textPanes[i], XmNheight, &textAreaHeight, NULL);
// TODO:       textHeight += textAreaHeight - 2*marginHeight;
// TODO:    }
// TODO:    borderHeight = windowHeight - textHeight;
// TODO: 
// TODO:    /* Calculate a new window height appropriate for the new font */
// TODO:    newWindowHeight = (textHeight*getFontHeight(window)) / oldFontHeight +
// TODO:                      borderHeight;
// TODO: 
// TODO:    /* Many window managers enforce window size increments even on client resize
// TODO:       requests.  Our height increment is probably wrong because it is still
// TODO:       set for the previous font.  Set the new height in advance, before
// TODO:       attempting to resize. */
// TODO:    XtVaSetValues(window->mainWindow, XmNheightInc, getFontHeight(window), NULL);
// TODO: 
// TODO:    /* Re-size the window */
// TODO:    XtVaSetValues(window->mainWindow, XmNheight, newWindowHeight, NULL);
}
