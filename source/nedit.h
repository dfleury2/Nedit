...
typedef struct _UserMenuListElement
{
   ...
   bool                  umleAccLockPatchApplied; /* indicates, if accelerator
                                                      lock patch is applied */
   Fl_Widget*            umleMenuItem;            /* menu item represented by
                                                      this element */
   Fl_Widget*            umleSubMenuPane;         /* holds menu pane, if item
...
typedef struct _UserMenuCache
{
   int          umcLanguageMode;     /* language mode applied for shared
                                         user menus */
   bool         umcShellMenuCreated; /* indicating, if all shell menu items
                                         were created */
   bool         umcMacroMenuCreated; /* indicating, if all macro menu items
                                         were created */
...
typedef struct _UserBGMenuCache
{
   int          ubmcLanguageMode;    /* language mode applied for background
                                         user menu */
   bool         ubmcMenuCreated;     /* indicating, if all background menu
                                         items were created */
...
struct  WindowInfo
{
   enum Type { Window, Document };

   WindowInfo* next;
   Fl_Double_Window*	mainWindow;    /* main window */
   Type        type; /* Window or Document ? */
   ...
   Ne_Text_Editor*	textPanes[MAX_PANES];	/* additional ones created on demand */
   ...
   int flashTimeoutID;	/* timer procedure id for getting rid
   ...
   WindowInfo**	writableWindows;/* temporary list of writable windows
