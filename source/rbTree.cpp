...
{
   TestNode* newNode = malloc__(sizeof(TestNode));
   if (newNode)
   {
      newNode->str = malloc__(strlen(((TestNode*)src)->str) + 1);
      if (newNode->str)
      {
         strcpy(newNode->str, ((TestNode*)src)->str);

         newNode->key = malloc__(strlen(((TestNode*)src)->key) + 1);
         if (newNode->key)
         {
...
rbTreeNode* rbTreeAllocateEmptyNodeCB_TestNode()
{
   TestNode* newNode = malloc__(sizeof(TestNode));
   if (newNode)
   {
...
static void rbTreeDisposeNode_TestNode(rbTreeNode* src)
{
   if (src)
   {
      if (((TestNode*)src)->str)
      {
         free__(((TestNode*)src)->str);
         ((TestNode*)src)->str = NULL;
      }
      if (((TestNode*)src)->key)
      {
         free__(((TestNode*)src)->key);
...
static int rbTreeCopyToNode_TestNode(rbTreeNode* dst, rbTreeNode* src)
{
   ...
   newValues.str = malloc__(strlen(((TestNode*)src)->str) + 1);
   if (newValues.str)
   {
      strcpy(newValues.str, ((TestNode*)src)->str);

      newValues.key = malloc__(strlen(((TestNode*)src)->key) + 1);
      if (newValues.key)
      {
