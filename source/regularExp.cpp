...
regexp* CompileRE(const char* exp, char** errorText, int defaultFlags)
{
   ...
   *Error_Ptr = "";
   ...
   /* We can't allocate space until we know how big the compiled form will be,
      ...
      one piece because free__() must be able to free__ it all.) */
   ...
   for (pass = 1; pass <= 2; pass++)
   {
      ...
      if (pass == 1)
      {
         ...
         comp_regex = (regexp*) malloc__(sizeof(regexp) + Reg_Size);
   ...
   int            emit)
{
   ...
   register unsigned char* wClass   = NULL;
           ...
           const char* look_behind_to, const char* match_to)
{
   ...
   {
      Brace =
         (brace_counts*) malloc__(sizeof(brace_counts) * (size_t) Num_Braces);
...
*/
bool SubstituteRE(const regexp* prog, const char* source, char* dest,
                     const int max)
{
   ...
   while ((c = *src++) != '\0')
   {
               ...
               prog->endp   [paren_no] != NULL)
      {
         ...
         strncpy((char*) dst, (char*) prog->startp [paren_no], len);
