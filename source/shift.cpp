...
static int findParagraphStart(Ne_Text_Buffer* buf, int startPos);
static int findParagraphEnd(Ne_Text_Buffer* buf, int startPos);
                      ...
                      int selStart, int selEnd, int rectStart, int rectEnd)
{
   ...
   Ne_Text_Buffer* tempBuf, *buf = window->buffer;
   char* text;
...
void FillSelection(WindowInfo* window)
{
   ...
   else
   {
      ...
      wrapMargin = window->textArea->wrapMargin;
...
** shifted text in memory that must be freed by the caller with free__.
*/
                ...
                int nChars, int* newLen)
{
   ...
   */
   ...
   shiftedText = (char*)malloc__(bufLen + 1);
                            ...
                            int tabDist, int nChars)
{
   ...
   lineInPtr = line;
   lineOut = (char*)malloc__(lineLen + nChars + 1);
   lineOutPtr = lineOut;
   whiteWidth = 0;
   while (true)
   {
...
static char* shiftLineLeft(char* line, int lineLen, int tabDist, int nChars)
{
   ...
   lineInPtr = line;
   lineOut = (char*)malloc__(lineLen + tabDist + 1);
   lineOutPtr = lineOut;
   whiteWidth = 0;
   lastWhiteWidth = 0;
   while (true)
                           ...
                           int* filledLen)
{
   ...
   cleanedText = (char*)malloc__(strlen(text)+1);
   outPtr = cleanedText;
   inMargin = true;
   for (c=text; *c!='\0'; c++)
   {
   ...
   *outPtr = '\0';
   ...
   outText = (char*)malloc__(sizeof(char) * (cleanedLen + leadIndentLen +
...
static char* makeIndentString(int indent, int tabDist, int allowTabs, int* nChars)
{
   ...
   outPtr = indentString = (char*)malloc__(sizeof(char) * indent + 1);
   if (allowTabs)
   {
