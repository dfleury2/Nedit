...
void Undo(WindowInfo* window)
{
   ...
   BufReplace(window->buffer, undo->startPos, undo->endPos, (undo->oldText != NULL ? undo->oldText : ""));
   ...
   /* free__ the undo record and remove it from the chain */
...
void Redo(WindowInfo* window)
{
   ...
   /* remove the redo record from the chain and free__ it */
...
static void addUndoItem(WindowInfo* window, UndoInfo* undo)
{
   ...
   if (window->undo == NULL)
   {
      SetSensitive(window, window->menuBar->getUndoItem(), true);
...
static void addRedoItem(WindowInfo* window, UndoInfo* redo)
{
   ...
   if (window->redo == NULL)
   {
      SetSensitive(window, window->menuBar->getRedoItem(), true);
...
/*
** Pop (remove and free__) the current (front) undo record from the undo list
*/
static void removeUndoItem(WindowInfo* window)
{
   UndoInfo* undo = window->undo;
   ...
   if (window->undo == NULL)
   {
      SetSensitive(window, window->menuBar->getUndoItem(), false);
      SetBGMenuUndoSensitivity(window, false);
   }
}

/*
** Pop (remove and free__) the current (front) redo record from the redo list
*/
static void removeRedoItem(WindowInfo* window)
{
   UndoInfo* redo = window->redo;
   ...
   if (window->redo == NULL)
   {
      SetSensitive(window, window->menuBar->getRedoItem(), false);
                              ...
                              int deletedLen, int direction)
{
   ...
   /* free__ the old saved text and attach the new */
   delete[] undo->oldText;
