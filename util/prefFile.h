...
struct PrefDescripRec
{
   ...
   char* defaultString;
   void* valueAddr; // int*, bool*, enum(int), or std::string*
   void* arg; // PREF_ENUM names
   bool save;
};

Ne_Database CreatePreferencesDatabase(const char* fileName, const char* appName);/*, XrmOptionDescList opTable, int nOptions, unsigned int* argcInOut, char** argvInOut);*/
...
int SavePreferences(const char* fileName, const  char* fileHeader, PrefDescripRec* rsrcDescrip, int nRsrc);
